...
public interface EngEventsViewRepository extends CrudRepository<EngEventsView, Integer>, JpaRepository<EngEventsView, Integer>{
   // @Query( value="SELECT * FROM ooeadmin.eng_events_view eev where eev.organization= :org ORDER BY ?#{#pageable}",
    //		countQuery = "SELECT count(*) FROM ooeadmin.eng_events_view eev where eev.organization= :org ORDER BY ?#{#pageable}"
    //		,nativeQuery=true)
	
    @Query( value="SELECT * from ooeadmin.ooeadmin.eng_events_view WHERE TRUNC(CAST(event_start_date_time as DATE))>= current_date ORDER BY ?#{#pageable}",
    		countQuery="SELECT count(*) from ooeadmin.ooeadmin.eng_events_view WHERE TRUNC(CAST(event_start_date_time as DATE))>= current_date ORDER BY ?#{#pageable}",
    		nativeQuery=true)
    public Page<EngEventsView> findByDefault(Pageable pageable);
	
    @Query( value="SELECT * from ooeadmin.ooeadmin.eng_events_view\r\n" + 
    		...
    		countQuery="SELECT count(*) from ooeadmin.ooeadmin.eng_events_view\r\n" + 
    	    		"                WHERE (TRUNC(CAST(event_start_date_time as DATE)) between :v_datefrom AND \r\n" + 
    	    		"                      :v_dateto) AND\r\n" + 
    	    		"                      county like :v_county AND\r\n" + 
    	    		"                      city like :v_city AND\r\n" + 
    ...
    @Query( value="SELECT * from ooeadmin.ooeadmin.eng_events_view where Lower(event_name) like LOWER('%'||:v_keyword||'%') OR LOWER(organization) like LOWER('%'||:v_keyword||'%') OR LOWER(department) like LOWER('%'||:v_keyword||'%') OR "
    		+ "LOWER(event_desc) like LOWER('%'||:v_keyword||'%') OR LOWER(event_type) like LOWER('%'||:v_keyword||'%') ORDER BY ?#{#pageable}",
    		countQuery="SELECT count(*) from ooeadmin.ooeadmin.eng_events_view where Lower(event_name) like LOWER('%'||:v_keyword||'%') OR LOWER(organization) like LOWER('%'||:v_keyword||'%') OR LOWER(department) like LOWER('%'||:v_keyword||'%') OR " + 
    				"LOWER(event_desc) like LOWER('%'||:v_keyword||'%') OR LOWER(event_type) like LOWER('%'||:v_keyword||'%') ORDER BY ?#{#pageable}",
    		nativeQuery=true)
